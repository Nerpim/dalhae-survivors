...
func start_wave_0() -> void:
	if enemy_spawner:
		var scenes: Array[PackedScene] = [kkomak_scene, warrior_kkomak_scene, fat_kkomak_scene]
...
func check_waves() -> void:
	...
	if progress >= 0.33 and not wave_1_done:
		wave_1_done = true
		start_wave_1()
	
	if progress >= 0.66 and not wave_2_done:
		wave_2_done = true
		start_wave_2()
...
func update_difficulty() -> void:
	...
	var new_interval = lerp(1.0, 0.3, progress)
